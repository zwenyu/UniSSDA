"""
Ensures all domains of a dataset have the same class distribution, then generates text files
...
if __name__ == "__main__":
    ...
    print("Number of samples per class")
    for c, num_samples in min_samples_dict.items():
        print(f"\t{c}: {num_samples}")
    ...
    for d in domains:
        ...
        for class_id, c in enumerate(class_list):
            retained_num_samples = min_samples_dict[c]
            retained_samples_paths = [os.path.relpath(f.path, data_dir) for f in os.scandir(os.path.join(data_dir, d, c))][
