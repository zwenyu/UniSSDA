...
def save_checkpoint(home_path, algorithm, selected_scenarios, dataset_configs, log_dir, hparams, ckpt_name='last_checkpoint'):
    save_dict = {
    ...
    save_path = os.path.join(home_path, log_dir, f'{ckpt_name}.pt')
...
def _calc_metrics(pred_labels, true_labels, log_dir, home_path, target_names, src_class, trg_class, eval_class='all',
    ckpt_name='last_checkpoint', split='test'):
    if eval_class == 'all':
        select_eval_class = trg_class
    elif eval_class == 'shared':
        select_eval_class = list(set(src_class) & set(trg_class))
    elif eval_class == 'trg_only':
        select_eval_class = list(set(trg_class) - set(src_class))
    ind_class = np.array([lab in select_eval_class for lab in list(true_labels[split])])
    if sum(ind_class) > 0:
        pred_labels = np.array(pred_labels[split][ind_class]).astype(int)
        true_labels = np.array(true_labels[split][ind_class]).astype(int)
        target_names = np.array(target_names)[np.unique(np.concatenate((pred_labels, true_labels))).astype(int)].tolist()
        ...
        # save classification report
        file_name = f"{ckpt_name}_train_classification_report_{eval_class}_class.xlsx" if (split == 'train') else f"{ckpt_name}_classification_report_{eval_class}_class.xlsx"
        report_Save_path = os.path.join(home_path, log_dir, file_name)
        df.to_excel(report_Save_path)

        return accuracy * 100, r["macro avg"]["f1-score"] * 100, r["weighted avg"]["f1-score"] * 100
    else:
        return -1, -1, -1
...
def calc_dev_risk(target_model, src_train_dl, tgt_train_dl, src_valid_dl, configs, device):
    src_train_feats = target_model.feature_extractor(src_train_dl.dataset.x_data.to(device))
    tgt_train_feats = target_model.feature_extractor(tgt_train_dl.dataset.x_data.to(device))
    src_valid_feats = target_model.feature_extractor(src_valid_dl.dataset.x_data.to(device))
...
def calculate_risk(target_model, risk_dataloader, device):
    cls_loss = 0
    for data in risk_dataloader:
        x_data, y_data = data[0], data[1]
        feat = target_model.feature_extractor(x_data.to(device))
        pred = target_model.classifier(feat)
        cls_loss += F.cross_entropy(pred, y_data.to(device)).item()
    cls_loss /= risk_dataloader.__len__()
    return cls_loss
